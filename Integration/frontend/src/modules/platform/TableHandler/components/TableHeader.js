...
class TableHeader extends React.Component {
  ...
  createTabs = () => {
    ...
    return this.props.tabs.map((conf, key) => {
      if (conf.isVisible) {
        ...
        return <Tab label={conf.label} key={key} value={conf.label} className={classes.tableHeaderTabberTab} />;
      }
    });
  };
  ...
  render() {
    ...
    const { classes, theme, localize, privilege } = this.props;
    return (
      <div className={classes.tableHeader}>
          ...
          <AppBar position="static" color="default" className={classes.tableHeaderPaper}>
            <div className={classes.tableHeaderToolbar}>
              <div className={classes.tableHeaderLeft}>
                ...
                {this.props.tipVisible && this.props.tip.isVisible && this.props.showTip && this.props.selectedRow.length === 0 ? (
                  <div className={classes.messageBox}>
                    <IconButton className={classes.lightBulbIcon} color="inherit" aria-label="Menu">
                      <SvgIcon viewBox="0 0 1000.000000 1000.000000">{lightBulbIcon}</SvgIcon>
                    </IconButton>
                    <Typography>{this.props.tip.label}</Typography>
                  </div>
                ) : null}
        ...
        {tabBarVisible > 1 && (
          <AppBar position="static" color="default" className={classes.tableHeaderPaper}>
            <div className={classes.tableHeaderToolbar}>
              <div className={classes.tableHeaderRight}>
                <Tabs value={selectedTab} onChange={this.handleTabChange} className={classes.tableHeaderTabs} textColor={theme.palette.primary2Color} fullWidth="true">
                  {this.createTabs()}
                </Tabs>
