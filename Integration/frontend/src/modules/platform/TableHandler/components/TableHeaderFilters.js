...
const TableHeaderFilters = ({
      ...
      switch (items.type) {
          ...
          if (items.icon == 'edit') {
            ...
            return (
              <div key={index} className={classes.subHeaderRightCol}>
                ...
                >
                  <IconButton onClick={toggleColumnEditorModal} className={classes.editIcon}>
                    <EditIcon />
                  </IconButton>
                </Tooltip>
              </div>
            ...
            return (
              <div key={index} className={classes.subHeaderRightCol}>
                {isSearchClick ? (
                  ...
                  >
                    <IconButton className={classes.searchIcon} onClick={e => handleSearch(e, true)}>
        ...
        case 'dropdown':
          ...
          const subheaderfilterlist = items.filter.map((conf, key) => {
            return (
              <MenuItem value={conf.name} key={'dropdown' + key}>
                {conf.name}
              </MenuItem>
            );
          });
          return (
            <div key={index} className={classes.subHeaderRightCol}>
              <FormControl className={classes.dropBox}>
                <Select value={selectedDropDown} className={classes.selectedDropDown} onChange={e => handleSubHeaderDropDown(e)}>
                  {subheaderfilterlist}
                </Select>
              </FormControl>
            </div>
        ...
        case 'button':
          ...
          if (selectedRow.length > 0) {
            ...
            if (items.value === 'DOWNLOAD')
              if (privilege.logs && !privilege.logs.download) return;
              else buttonClass = classes.raisedButton;
            else if (items.value === 'CANCEL') buttonClass = classes.flatButton;
            return (
  ...
  if (flag) {
    return <div className={classes.subHeaderRight}>{getSubheader}</div>;
  } else {
    return <div />;
  }
};
