...
export default class ComponentLocator extends Component {
  ...
  drawCircles = () => {
    ...
    function dragged(d, i) {
      ...
      d3.select(this).attr('transform', `translate(${(that.props.circles[i].xAxis = d.xAxis = x)},${(that.props.circles[i].yAxis = d.yAxis = y)})`);
    }
    ...
    const handleDeleteClick = () => {
      ...
      changeVisible(currentIndex, false);
      tooltip.style('opacity', 0);
    };
    ...
    if (imageURL) {
      const circle = svg
        ...
        .on('click', (d, i) => {
          ...
          currentIndex = i;
          tooltip
            .html(`${d.componentName}<br/>${d.uniqueId}<div class="removeButton">REMOVE</div>`)
            .style('display', 'flex')
            .style('height', 'auto')
  ...
  render() {
    ...
    return (
      <div>
        <div style={{ width: 0, height: 0 }} id="tooltipWrapper">
          <div className="tooltip" style={{ display: 'flex', flexDirection: 'row' }}>
            <div>REMOVE</div>
          </div>
        </div>
