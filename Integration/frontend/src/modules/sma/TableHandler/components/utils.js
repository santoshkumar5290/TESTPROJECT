 ...
 */
export const  _getTextLength = (input, type = '') => {
  let valueData = ''
  ...
  if (type === 'LONGSTRING' && input[0] !== null) {
    ...
    return calculatePixelSize(valueData).width + 2 * valueData.length
  } else if (type === 'DROPDOWN') {
    return calculatePixelSize(input[0]).width + 2 * input[0].length
  } else if (type === 'CHECKBOX') {
    // 50 added for checkbox size
    return calculatePixelSize(input).width + 2 * input.length + 50
  } else {
    return calculatePixelSize(input).width + 2 * input.length
  }
};
...
export const setMinWidth = (columns) => {
  ...
  for (let i = 0; i < columns.length; i++) {
    ...
    if (columns[i].type === 'CHECKBOX') {
      minWidth[columns[i].property] = max + 105
