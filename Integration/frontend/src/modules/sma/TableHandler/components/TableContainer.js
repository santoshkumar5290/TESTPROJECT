...
*/
  sortBase = (dir, item, parentIndex) => {
    ...
    const length =
      parentIndex >= 0
        ? cards.get(parentIndex).get('children').size
        : cards.size
    if (
      (dir === 'up' && item.get('index') === 0) ||
      (dir === 'down' && item.get('index') === length - 1)
    ) {
      return
    }
  ...
  updateBool = (index, label, parentIndex, prop) => {
    ...
    if(label === localize("PERCENTAGE")){
      ...
      updated = updateBool({ cards, index, parentIndex, prop })
...
*/
  ...
  render () {
    const { localizationSet, classes, localize, emptyListMsg } = this.props
    return (
      <Paper className={classes.wrapperPaper} id="tablePaper">
        ...
        {this.state.modalOpen ? (
          <Dialog
            ...
            classes={{ 'paper': this.props.classes.modalEditPaper }}
            onClose={this.cancelTableDisplaySettings} >
            <DialogTitle className={classes.modalEditHeader} id='edit-dialog-title'>{localize('COLUMN_ARRANGEMENT')}</DialogTitle>
