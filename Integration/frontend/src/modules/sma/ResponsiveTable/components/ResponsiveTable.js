...
class ResponsiveTable extends React.Component {
  ...
  calculateIsHidden () {
    ...
    for (var j = 0; j < columns.length; j++) {
      ...
      var cell =
        row &&
        row.getElementsByClassName(columns[j].props.className)[0] &&
        row.getElementsByClassName(columns[j].props.className)[0].getElementsByClassName('value_')[0]
   ...
   */
  render () {
    const { components, classes, pagination, pageSize, page, rowCount, localize } = this.props
    const { columns, rows } = this.state
    const tableStyle = classnames(
      classes.responsiveTableWidget,
          ...
          <TablePagination
            ...
            labelDisplayedRows={function () {
              return pageSize === 'All'
                ...
                : `${rowCount === 0 ? 0 : page * pageSize + 1}-${Math.min(
                    rowCount,
                    (page + 1) * pageSize
                  )} of ${rowCount}`
            }}
            rowsPerPageOptions={[10, 25, 100, 300, 'All']}
